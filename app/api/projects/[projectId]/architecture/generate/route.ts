...
export async function POST(
  ...
  try {
    ...
    // Prevent duplicate Architecture
    const existingArchitecture =
      await prisma.architecture.findUnique({
        where: {
          roadmapId,
        },
      });
    ...
    if (existingArchitecture) {
      return NextResponse.json(
        {
          message: "Architecture already exists for this roadmap.",
    ...
    if (error?.status === 503) {
      return NextResponse.json(
        {
          message:
            "Gemini AI is currently busy. Please try again in a moment.",
        },
        {
          status: 503,
        }
