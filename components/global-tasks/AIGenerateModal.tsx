      ...
      {open && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-white/10 bg-[#090909] shadow-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-white/[0.07] px-8 py-5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04]">
                  <Brain size={18} className="text-orange-400/90" />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-white" style={{ fontFamily: "var(--font-sora)" }}>
                    AI Task Generator
                  </h2>
                  <p className="text-xs text-[#8a8a93]">Powered by Gemini</p>
                </div>
              </div>
              <button
                onClick={handleClose}
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] text-[#8a8a93] transition hover:text-white"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="flex border-b border-white/[0.07] bg-white/[0.01]">
              {(["input", "preview"] as const).map((s, i) => (
                <div
                  key={s}
                  className={`flex flex-1 items-center gap-2.5 px-8 py-3 text-xs font-semibold transition ${
                    step === s
                ...
                >
                  ...
                  <span>{s === "input" ? "Describe Goal" : "Review & Approve"}</span>
                  {i === 0 && <ChevronRight size={14} className="text-[#8a8a93]/60 ml-auto" />}
                </div>
              ))}
            </div>
            ...
            <div className="px-8 pb-8 pt-6">
              {step === "input" ? (
                <div className="space-y-6">
                  {/* Project selector */}
                  <div>
                    <label className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-[#8a8a93]">
                      <FolderKanban size={12} /> Target Project *
                    </label>
                    <select
                      value={projectId}
                      onChange={(e) => setProjectId(e.target.value)}
                      className="w-full rounded-2xl border border-white/10 bg-[#111] px-4 py-3 text-sm text-white outline-none focus:border-white/20"
                    >
                  ...
                  {/* Prompt */}
                  <div>
                    <label className="mb-2 block text-xs font-semibold text-[#8a8a93]">
                    ...
                    <textarea
                      ...
                      className="w-full resize-none rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white placeholder-[#8a8a93] outline-none focus:border-white/20"
                    />
                  </div>

                  {/* Examples */}
                  <div>
                    <span className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-[#8a8a93]">
                      Try an example
                    </span>
                    <div className="flex flex-wrap gap-2">
                      {EXAMPLE_PROMPTS.map((p) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setPrompt(p)}
                          className="rounded-xl border border-white/10 bg-white/[0.02] px-3.5 py-1.5 text-xs text-[#8a8a93] transition hover:border-orange-500/30 hover:bg-orange-500/[0.04] hover:text-orange-300"
                        >
                          {p}
                        </button>
                      ))}
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2">
                    <button
                      type="button"
                      onClick={handleClose}
                      className="rounded-2xl border border-white/10 px-5 py-2.5 text-sm font-medium text-[#8a8a93] transition hover:bg-white/[0.05] hover:text-white"
                    >
                      Cancel
                    </button>
                    ...
                    <button
                      ...
                      className="btn-shimmer flex items-center gap-2 rounded-2xl bg-white px-6 py-2.5 text-sm font-semibold text-black shadow-sm transition hover:bg-zinc-200 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
                    >
                      {generating ? (
                ...
                <div className="space-y-5">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="mt-0.5 text-xs text-[#8a8a93] line-clamp-2">{task.description}</p>
                            <div className="mt-2 flex items-center gap-2 flex-wrap">
                              <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold capitalize ${pCfg.bg} ${pCfg.color}`}>
                                {task.priority}
                              </span>
                              {task.estimatedHours && (
                                <span className="text-[10px] text-[#8a8a93]">~{task.estimatedHours}h</span>
                              )}
                              {(task.tags || []).map((t) => (
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <button
                      ...
                      className="text-xs text-[#8a8a93] transition hover:text-white"
                    >
                      ← Back
                    </button>
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="btn-shimmer flex items-center gap-2 rounded-2xl bg-white px-6 py-2.5 text-sm font-semibold text-black transition hover:bg-zinc-200 disabled:opacity-50 active:scale-95"
                      >
                        {saving ? <Loader2 size={16} className="animate-spin" /> : <ArrowRight size={16} />}
                        {saving ? "Saving..." : `Save ${selectedTaskIds.size} Task${selectedTaskIds.size !== 1 ? "s" : ""}`}
                      </button>
                    </div>
                  </div>
                </div>
